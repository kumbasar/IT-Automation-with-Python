# Google IT Automation with Python 

My workspace for [Google IT Automation with Python](https://www.coursera.org/professional-certificates/google-it-automation)

## Projects
- [Log CSV reporter](LogParsing/)
- [Daily Sync](Backup/)
- [Employees' start date report](Start-Date-Report/)
- [Manipulating Images](Manipulating-Images/)
- [Interacting with Web Services](Interacting-with-Web-Services/)
- [Automatically Generate a PDF and send it by Email](Automatically-Generate-a-PDF-and-send-it-by-Email/)
- [Automate updating catalog information](Automate-updating-catalog-information/)
